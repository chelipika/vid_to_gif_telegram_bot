...
# Use pathlib for download directory
...
async def convert_now(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        output = download_dir / f"{video_id}_{user_id}.gif"
        file_path = download_dir / f"{video_id}_{user_id}.mp4"

        await bot.download(video_id, destination=str(file_path))
        ...
        gif = FSInputFile(str(output), f"{video_id}_{user_id}.gif")
        await callback.message.answer_animation(gif)
        
        # Clean up using pathlib's unlink
